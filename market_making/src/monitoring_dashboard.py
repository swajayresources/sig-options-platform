...
<head>
    ...
    <style>
        ...
        .alert-warning { border-left-color: #feca57; }
...
<body>
    ...
    <script>
        ...
        function updateChart(chartType, timeframe) {
            ...
            fetch(`/api/charts/${chartType}?timeframe=${timeframe}&strategy_id=all`)
        ...
        function loadInitialData() {
            ...
            fetch('/api/overview')
                .then(response => response.json())
                .then(data => updateOverviewMetrics(data));

            // Load charts
            fetch('/api/charts/pnl?timeframe=1D&strategy_id=all')
                .then(response => response.json())
                .then(data => Plotly.newPlot('pnl-chart', data.data, data.layout));

            fetch('/api/charts/greeks?timeframe=1D&strategy_id=all')
                .then(response => response.json())
                .then(data => Plotly.newPlot('greeks-chart', data.data, data.layout));

            fetch('/api/charts/positions?strategy_id=all')
                .then(response => response.json())
                .then(data => Plotly.newPlot('positions-chart', data.data, data.layout));

            fetch('/api/charts/risk_gauge')
                .then(response => response.json())
                .then(data => Plotly.newPlot('risk-gauge', data.data, data.layout));
